...
# NPSClassify - AI customer-feedback classifier on GenLayer.
...
class NPSClassify(gl.Contract):
    ...
    def evaluate(self, item_id: int) -> None:
        ...
        def leader_fn():
            ...
            prompt = f"""You classify NPS-style customer feedback. Classify the FEEDBACK as promoter, passive, or detractor.
...
Classify the CONTENT as exactly one of: promoter, passive, detractor.
Also give an integer sentiment score from 0 to 100, and a one-sentence reason.

Respond with ONLY JSON: {{"label": "<one of: promoter, passive, detractor>", "score": <integer 0-100>, "reasoning": "<one sentence>"}}"""
