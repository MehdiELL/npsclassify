...
### AI customer-feedback classifier on GenLayer
...
Submit text; GenLayer validators independently evaluate it with an LLM, agree on a label and score by consensus, and write the result on chain. AI customer-feedback classifier.

**Deployed contract (Testnet Bradbury):** [`0x97530CfAF90A3e5600a01904d91483462cD70933`](https://explorer-bradbury.genlayer.com/address/0x97530CfAF90A3e5600a01904d91483462cD70933)
...
2. **`evaluate(id)`** - each validator classifies the content
   (one of: promoter, passive, detractor) and scores it 0-100, then must **agree** on the label and a similar
   score via the Equivalence Principle.
3. The **label, score, and reasoning** are written on chain.

## The Intelligent Contract

`contracts/npsclassify.py` targets the GenVM Python runner (pinned by hash):

- **Integers only** - the sentiment score is `u8` (0-100); no floats.
...
genvm-lint check contracts/npsclassify.py
